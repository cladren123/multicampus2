...
class implicitActivity : AppCompatActivity() {
    ...
    fun runPermission(){
        ...
        for(permission in permission_list){
            ...
            if(chk == PackageManager.PERMISSION_DENIED){
                // 권한 부여 요청 대화상자를 실행해준다. 사용자가 권한승인을 직접 하지만 대화상자는 시스템이 실행한다
    ...
    // 기존 방식은 앱을 설치하는 시점에만 권한에 대한 승인을 체크하는 방식이었으나 이 권한들이 단말기의 주요한 기능을
    // 사용할 수 있어서 위험권한으로 분류된 권한은 앱을 설치할 때가 아니라 사용할 때 권한을 승인하도록 변경
    ...
    fun runGoogleMap(v: View){
        // 실행할 data
        var uri = Uri.parse("geo:37.50398039671862, 127.04278665770238")

        // 인텐트가 인식할 action 명을 주고 인텐트 객체 생성
        // ACTION_VIEW 는 뭔가를 보여주는 목적으로 실행되는 앱에 줄 수 있는 action명
        // ACTION_VIEW 라는 이름을 안드로이드 OS에게 요청하면 미리 정리해 둔 컴포넌트 목록에서
        // ACTION_VIEW 라는 이름으로 등록된 모든 뷰를 찾아서 보여준다.
    ...
    fun runDial(v:View)
    {
        var uri = Uri.parse("tel:01050268498")
    ...
    fun runCallPhone(v:View)
    {
        ...
        val check = PermissionChecker.checkSelfPermission(this,
