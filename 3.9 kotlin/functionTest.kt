...
{
    /*
        1) 코틀린에서 함수를 정의한 자바로 변환될때 클래스의 메소드로 변환
        2) 함수 - 자바스크립트, C언어와 동일한 방법 동일한 개념으로 선언하고 사용할 수 있다.
        3) 형식
        fun 함수명(매개변수) : 변환값의 타입{
            함수내부에서 처리할 명령문
            [return]
        }

        4) 변환값의 타입은 지원하는 데이터타입, 변환값이 없는 경우 (Unit - void와 동일)
           Unit은 생략 가능하다.

        5) 함수를 호출할 때 변수명에 값을 할당할 수 있기 때문에 순서는 달라도 상관없다.
        6) 함수를 정의할 때 매개변수에 초기값을 지정할 수 있다.
